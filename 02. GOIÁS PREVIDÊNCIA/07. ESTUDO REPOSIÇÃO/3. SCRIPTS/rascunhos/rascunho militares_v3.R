        ...
        if(tempo_serv_pub_2021 < 30) {
          ...
          if(tempo_carreira_2021 >= 25) {
            ...
            if(is.na(TEMPO_SERV_PUB) != TRUE) {
              ...
              apos_serv_pub = as.Date(cargos_homens_oficiais[[i]]$DT_ING_SERV_PUB[k]) %m+% 
            ...
            if(is.na(TEMPO_CARREIRA) != TRUE) {
              ...
              apos_tempo_carreira = as.Date(cargos_homens_oficiais[[i]]$DT_ING_CARREIRA[k]) %m+% 
                                 years(ano) %m+% months(mes) %m+% days(dia)
            }
            ...
            if(is.na(TEMPO_SERV_PUB) != TRUE &&
              ...
              if(is.na(TEMPO_SERV_PUB) != TRUE) {
                apos_serv_pub = seq.Date(as.Date(cargos_homens_oficiais[[i]]$DT_ING_SERV_PUB[k]),
                                         by = "35 years",
                                         length.out = 2)[2]
              }
              ...
              if(is.na(TEMPO_CARREIRA) != TRUE) {
                apos_tempo_carreira = seq.Date(as.Date(cargos_homens_oficiais[[i]]$DT_ING_CARREIRA[k]),
                                               by = "30 years",
                                               length.out = 2)[2]
              }
...
masculino_oficiais = bind_rows(cargos_homens_oficiais)
