...
#Ativos
arquivos_anterior_ativos = read_excel("Atuario202301.xlsx", sheet = "Ativo")
arquivos_atual_ativos = read_excel("Atuario202302.xlsx", sheet = "Ativo")

# arquivos_anterior_ativos$IN_PARID_SERV = as.numeric(arquivos_anterior_ativos$IN_PARID_SERV)
# arquivos_atual_ativos$IN_PARID_SERV = as.numeric(arquivos_atual_ativos$IN_PARID_SERV)


ativos = arquivos_anterior_ativos %>%
...
ativos_massa = arquivos_anterior_ativos %>%
  ...
  filter(massa == TRUE); ativos_massa #quando TRUE, quer dizer que ha diferença entre a base de 
                                     #janeiro 2023 e a base de fevereiro 2023
...
ativos_tipo_cargo = arquivos_anterior_ativos %>%
  inner_join(arquivos_atual_ativos, by = "ID_SERVIDOR_CPF") %>%
  group_by(ID_SERVIDOR_CPF) %>%
  summarise(tipo_cargo = CO_TIPO_CARGO.y != CO_TIPO_CARGO.x) %>%
  filter(tipo_cargo == TRUE); ativos_tipo_cargo

ativos_criterio_elegibilidade = arquivos_anterior_ativos %>%
  inner_join(arquivos_atual_ativos, by = "ID_SERVIDOR_CPF") %>%
  group_by(ID_SERVIDOR_CPF) %>%
...
ativos_servidor_matricula = arquivos_anterior_ativos %>%
  inner_join(arquivos_atual_ativos, by = "ID_SERVIDOR_CPF") %>%
  group_by(ID_SERVIDOR_CPF) %>%
...
ativos_est_civil = arquivos_anterior_ativos %>%
  inner_join(arquivos_atual_ativos, by = "ID_SERVIDOR_CPF") %>%
  group_by(ID_SERVIDOR_CPF) %>%
...
ativos_situacao_funcional = arquivos_anterior_ativos %>%
  inner_join(arquivos_atual_ativos, by = "ID_SERVIDOR_CPF") %>%
  group_by(ID_SERVIDOR_CPF) %>%
...
#Inativos
rm(list=ls())
arquivos_anterior_inativos = read_excel("Atuario202301.xlsx", sheet = "Inativo")
arquivos_atual_inativos = read_excel("Atuario202302.xlsx", sheet = "Inativo")

inativos = arquivos_anterior_inativos %>%
...
inativos_fundo = arquivos_anterior_inativos %>%
  inner_join(arquivos_atual_inativos, by = "ID_APOSENTADO_CPF") %>%
  group_by(ID_APOSENTADO_CPF) %>%
  summarise(fundo = CO_TIPO_FUNDO.y != CO_TIPO_FUNDO.x) %>%
  filter(fundo == TRUE); inativos_fundo; inativos_fundo

inativos_cnpj_orgao = arquivos_anterior_inativos %>%
  inner_join(arquivos_atual_inativos, by = "ID_APOSENTADO_CPF") %>%
  group_by(ID_APOSENTADO_CPF) %>%
...
inativos_tipo_poder = arquivos_anterior_inativos %>%
  inner_join(arquivos_atual_inativos, by = "ID_APOSENTADO_CPF") %>%
  group_by(ID_APOSENTADO_CPF) %>%
  summarise(tipo_poder = CO_TIPO_PODER.y != CO_TIPO_PODER.x) %>%
...
inativos_tipo_cargo = arquivos_anterior_inativos %>%
  inner_join(arquivos_atual_inativos, by = "ID_APOSENTADO_CPF") %>%
  group_by(ID_APOSENTADO_CPF) %>%
  summarise(tipo_cargo = CO_TIPO_CARGO.y != CO_TIPO_CARGO.x) %>%
  filter(tipo_cargo == TRUE); inativos_tipo_cargo

inativos_tipo_APOSENTADOria = arquivos_anterior_inativos %>%
  inner_join(arquivos_atual_inativos, by = "ID_APOSENTADO_CPF") %>%
  group_by(ID_APOSENTADO_CPF) %>%
...
inativos_APOSENTADO_matricula = arquivos_anterior_inativos %>%
  inner_join(arquivos_atual_inativos, by = "ID_APOSENTADO_CPF") %>%
  group_by(ID_APOSENTADO_CPF) %>%
...
inativos_inicio_aposentadoria = arquivos_anterior_inativos %>%
  inner_join(arquivos_atual_inativos, by = "ID_APOSENTADO_CPF") %>%
  group_by(ID_APOSENTADO_CPF) %>%
...
inativos_teto_especifico = arquivos_anterior_inativos %>%
  inner_join(arquivos_atual_inativos, by = "ID_APOSENTADO_CPF") %>%
  group_by(ID_APOSENTADO_CPF) %>%
  summarise(teto = VL_TETO_ESPECIFICO.y != VL_TETO_ESPECIFICO.x) %>%
