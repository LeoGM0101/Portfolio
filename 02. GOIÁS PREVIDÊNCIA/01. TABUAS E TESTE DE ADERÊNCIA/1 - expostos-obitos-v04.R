...
setwd("Z:/GERENCIA/RELATÓRIO DE HIPÓTESES/2023/RPPS/MORTALIDADE/01. TABUAS E TESTE DE ADERÊNCIA")
base2018_ativos = read_excel("base2018.xlsx", sheet = "Ativo") %>%  filter(militar == "N")
base2018_inativos = read_excel("base2018.xlsx", sheet = "Inativo") %>%  filter(TipoAposentadoria != "Invalidez", Militar == "N")
base2019 = read_excel("base2019.xlsx")
base2020 = read_excel("base2020.xlsx")
base2021 = read_excel("base2021.xlsx")
base2022_ativos = read_excel("base2022.xlsx", sheet = "Ativo") %>% filter(CO_COMP_MASSA == 1)
base2022_inativos = read_excel("base2022.xlsx", sheet = "Inativo") %>% filter(CO_TIPO_APOSENTADORIA != 4, CO_COMP_MASSA == 1) #Retirando os militares da base 2022 e 2023 e retirando a aposentadoria por invalidez
base2023_ativos = read_excel("base2023.xlsx", sheet = "Ativo") %>% filter(CO_COMP_MASSA == 1)
base2023_inativos = read_excel("base2023.xlsx", sheet = "Inativo") %>% filter(CO_TIPO_APOSENTADORIA != 4, CO_COMP_MASSA == 1)
obitos2018 = read_excel("OBITOS_GPREV_17102023_v02.xlsx") %>% filter(ANO_OBITO == "2018", MILITAR == 2) #Retirando os militares da tabela de obitos
obitos2019 = read_excel("OBITOS_GPREV_17102023_v02.xlsx") %>% filter(ANO_OBITO == "2019", MILITAR == 2)
obitos2020 = read_excel("OBITOS_GPREV_17102023_v02.xlsx") %>% filter(ANO_OBITO == "2020", MILITAR == 2) 
obitos2021 = read_excel("OBITOS_GPREV_17102023_v02.xlsx") %>% filter(ANO_OBITO == "2021", MILITAR == 2) 
obitos2022 = read_excel("OBITOS_GPREV_17102023_v02.xlsx") %>% filter(ANO_OBITO == "2022", MILITAR == 2) 
obitos2023 = read_excel("OBITOS_GPREV_17102023_v02.xlsx") %>% filter(ANO_OBITO == "2023", MILITAR == 2)
#------------------------------------------------------------------------------#
#Expostos e obitos no ano de 2018

#TipoAposentadoria = 0 -> Tipo de Aposentadoria definida como null na base

tabela_2018_ativos_masculino = base2018_ativos %>%
  filter(sexo == "M", militar == "N", idade <= 75) %>% #Limitou a idade dos ativos ate 75 anos devido a aposentadoria compulsoria
  group_by(idade) %>%
  distinct(cpf) %>%
  dplyr::count(idade) %>%
...
tabela_2018_inativos_masculino = base2018_inativos %>%
  replace(is.null(.), 0) %>%
  filter(TipoAposentadoria != "Invalidez", sexo == "M", Militar == "N") %>%
  group_by(idade) %>%
  distinct(cpf) %>%
  dplyr::count(idade) %>%
  dplyr::rename(QUANTIDADE = n, IDADE = idade)
...
tabela_2018_masculino_obitos = obitos2018 %>%
  filter(CO_SEXO_SERVIDOR == 2, MILITAR == 2) %>%
...
tabela_2018_ativos_feminino = base2018_ativos %>%
  filter(sexo == "F", militar == "N", idade <= 75) %>%
  group_by(idade) %>%
  distinct(cpf) %>%
  dplyr::count(idade) %>%
  dplyr::rename(QUANTIDADE = n, IDADE = idade)
...
tabela_2018_inativos_feminino = base2018_inativos %>%
  replace(is.null(.), 0) %>%
  filter(TipoAposentadoria != "Invalidez", sexo == "F", Militar == 2) %>%
  group_by(idade) %>%
  distinct(cpf) %>%
  dplyr::count(idade) %>%
  dplyr::rename(QUANTIDADE = n, IDADE = idade)
...
tabela_2018_feminino_obitos = obitos2018 %>%
  filter(CO_SEXO_SERVIDOR == 1, MILITAR == 2) %>%
  group_by(IDADE_OBITO) %>%
  distinct(ID_SERVIDOR_CPF) %>%
  dplyr::count(IDADE_OBITO) %>%
  dplyr::rename(QUANTIDADE = n)
...
tabela_2019_feminino_obitos = obitos2019 %>%
  filter(CO_SEXO_SERVIDOR == 1, IDADE_OBITO <= 103, MILITAR == 2) %>% #Limite para a idade de obito em 103 devido a erro na data de nascimento
  group_by(IDADE_OBITO) %>%
  distinct(ID_SERVIDOR_CPF) %>%
  dplyr::count(IDADE_OBITO) %>%
  dplyr::rename(QUANTIDADE = n)
...
tabela_2022_inativos_masculino = base2022_inativos %>%
  replace(is.null(.), 0) %>%
  filter(CO_TIPO_APOSENTADORIA != 4, CO_SEXO_APOSENTADO == 2) %>%
  group_by(idade) %>%
...
tabela_2022_inativos_feminino = base2022_inativos %>%
  replace(is.null(.), 0) %>%
  filter(CO_TIPO_APOSENTADORIA != 4, CO_SEXO_APOSENTADO == 1) %>%
  group_by(idade) %>%
  distinct(ID_APOSENTADO_CPF) %>%
  dplyr::count(idade) %>%
  dplyr::rename(QUANTIDADE = n, IDADE = idade)
...
openxlsx::write.xlsx(lista_soma, file="Z:/GERENCIA/RELATÓRIO DE HIPÓTESES/2023/RPPS/MORTALIDADE/01. TABUAS E TESTE DE ADERÊNCIA/expostos e obitos.xlsx")
